...
impl LoweringContext {
            ...
            if let crate::ast::Item::Function(func) = item {
                if func.name.name == "main" {
                    for param in &func.params {
                        ...
                        self.quantities.insert(param.name.name.clone(), param.ty.quantity);
                    }
                    break;
                }
            }
    ...
    fn lower_let_inout(&mut self, let_inout: &crate::ast::LetInOutStmt) -> Result<(), LoweringError> {
        // Track quantity
    ...
    fn lower_let_consume(&mut self, let_consume: &crate::ast::LetConsumeStmt) -> Result<(), LoweringError> {
        // Track quantity
    ...
    fn lower_reversible_block(&mut self, block: &crate::ast::expr::ReversibleBlock) -> Result<(), LoweringError> {
    ...
    fn lower_expr(&mut self, expr: &crate::ast::Expr) -> Result<crate::ir::PirExpr, LoweringError> {
        ...
        match &expr.kind {
            ...
            ExprKind::Binary(op, left, right) => {
                ...
                Ok(PirExpr::Binary { op: self.lower_binop(*op), left: Box::new(l), right: Box::new(r) })
            }
            ExprKind::Unary(op, expr) => {
                let e = self.lower_expr(expr)?;
                Ok(PirExpr::Unary { op: self.lower_unop(*op), expr: Box::new(e) })
            }
            ...
            ExprKind::Index(base, index) => {
                ...
                Ok(PirExpr::Index { base: Box::new(b), indices: vec![idx] })
            }
            ExprKind::Field(base, field) => {
                let b = self.lower_expr(base)?;
                Ok(PirExpr::Field { base: Box::new(b), field: field.name.clone() })
            }
            ...
            ExprKind::If(cond, then_branch, else_branch) => {
                ...
                let e = else_branch.as_ref().map(|b| self.lower_expr(b)).transpose()?.unwrap_or(PirExpr::IntLit(0));
                Ok(PirExpr::If { cond: Box::new(c), then_branch: Box::new(t), else_branch: Box::new(e) })
            }
            ExprKind::Reversible(block) => {
                // Lower reversible block expression
                let b = self.lower_reversible_expr(block)?;
                // For now just return a placeholder
                Ok(PirExpr::Reversible { body: Box::new(b), inverse: Box::new(PirExpr::IntLit(0)) })
            }
            _ => Err(LoweringError::Unsupported(format!("{:?}", expr.kind))),
        }
    }
    
    fn lower_literal(&self, lit: &crate::ast::Literal) -> Result<crate::ir::PirExpr, LoweringError> {
    ...
    fn lower_reversible_expr(&mut self, block: &crate::ast::expr::ReversibleBlock) -> Result<crate::ir::PirExpr, LoweringError> {
