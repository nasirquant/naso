...
pub struct HoverInfo {
    ...
    pub quantity: Option<naso_compiler::Quantity>,
...
impl CompilerBridge {
    ...
    pub async fn get_hover(&self, uri: &Url, position: Position) -> Option<HoverInfo> {
        ...
        if let Some(cached) = self.analysis_cache.get(uri) {
            cached.hover_info.get(&key).cloned()
        } else {
            // Perform analysis if not cached
            let document = self.document_store.get(uri)?;
            let result = self.perform_analysis(&document).await;
            let hover = result.hover_info.get(&key).cloned();
    ...
    fn generate_hover_info(
        ...
        let quantum_intrinsics: HashMap<&str, (&str, naso_compiler::Quantity, &str)> = [
            ...
            ("qfree", ("()", naso_compiler::Quantity::Many, "Explicitly free a qubit (triggers uncomputation). Use only when automatic uncomputation is insufficient.")),
            ("hadamard", ("Qubit -> Qubit", naso_compiler::Quantity::One, "Apply Hadamard gate H = 1/√2 [[1, 1], [1, -1]]. Consumes input qubit linearly.")),
