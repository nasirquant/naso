...
pub fn type_error_to_diagnostics(
    ...
    let (message, severity, code, related_ranges) = match error {
        TypeError::LinearVariableUsedTwice {
            name,
            first_use,
            second_use,
        } => {
            let message = format!(
                "linear variable `{}` used twice (first at {:?}, second at {:?})",
                name, first_use, second_use
            );
        ...
        TypeError::UnusedLinearVariable { name, defined_at } => {
            let message = format!(
                "unused linear variable `{}` (defined at {:?})",
                name, defined_at
            );
        ...
        TypeError::QuantityMismatch {
            ...
            if matches!(expected, naso_compiler::Quantity::One)
                || matches!(found, naso_compiler::Quantity::One)
            {
