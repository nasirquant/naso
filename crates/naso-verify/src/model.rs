        ...
        if let Some(m) = z3_model {
            ...
            for decl in m.get_func_decls() {
                ...
                if decl.arity() == 0 {
                    ...
                    // Function - extract interpretation
...
pub enum ModelValue {
    ...
    Real(String),
    Array(Vec<(ModelValue, ModelValue)>), // (index, value) pairs
    Uninterpreted(String),                // For uninterpreted sorts
}
...
impl ModelValue {
    fn from_z3(value: &z3::ast::Ast<'_>) -> Result<Self, String> {
        ...
        match kind {
            ...
            z3::SortKind::Real => {
                // Real values are returned as strings like "1/2" or "3.14"
                Ok(ModelValue::Real(value.to_string()))
            }
            z3::SortKind::Array => {
                // Array interpretation is complex; simplified for now
                Ok(ModelValue::Array(Vec::new()))
            }
...
impl FuncInterpretation {
    fn from_z3(model: &z3::Model, decl: &z3::FuncDecl<'_>) -> Result<Self, String> {
        // Z3 function interpretations are complex; simplified for now
        Ok(Self {
            entries: Vec::new(),
            else_branch: None,
        })
...
impl UnsatCore {
    ...
    pub fn from_z3(
        ...
        if let Some(core) = z3_core {
            // Map Z3 ASTs back to assertion names
            // This is simplified; real implementation would track named assertions
            for ast in core {
                // Find the assertion name by checking the AST
                // For now, just use the AST string
...
impl VerifyDiagnostic {
    /// Create from a verification result.
    pub fn from_verify_result(
        ...
        match result {
            crate::solver::VerifyResult::Unsat(Some(core)) => {
                Some(Self {
                    code: code.to_string(),
                    ...
                    related: core
                        ...
                        .iter()
                        .map(|a| RelatedInfo {
                            span: Span::dummy(), // Would need actual spans
            ...
            crate::solver::VerifyResult::Sat(model) => {
                // SAT means the property is violated (we assert the negation)
                Some(Self {
                    code: code.to_string(),
